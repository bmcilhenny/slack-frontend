...
class ChannelContainer extends React.Component {
  ...
  renderChannelMessages() {
    ...
    if (activeChannel && activeChannel.readMessages.length) {
      debugger
      activeChannelReadMessages = activeChannel.readMessages.map(message => <li className="formattedAvatarImg" key={message.message.id} id={message.message.id}><Image src={message.message.user.image_url} className="noBorderRadius" avatar/> <span className="bold">{message.message.user.display_name}</span> <span className="italic">{adapter.helpers.formatDateTime(message.message.created_at)}</span>: {message.message.content} </li>)
    }

    if (activeChannel && activeChannel.unreadMessages.length) {
      debugger
      activeChannelUnreadMessages = activeChannel.unreadMessages.map(message => <li className="formattedAvatarImg" key={message.message.id} id={message.message.id}> <Image src={message.message.user.image_url} className="noBorderRadius" avatar/><span className="bold">{message.message.user.display_name}</span> <span className="italic">{adapter.helpers.formatDateTime(message.message.created_at)}</span>: {message.message.content} </li>)
